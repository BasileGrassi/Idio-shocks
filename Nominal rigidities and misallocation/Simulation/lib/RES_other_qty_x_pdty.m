function RES = RES_other_qty_x_pdty(M,P,grid,p_fun,weight,params)
    ...
    mu = (profit_qty_x_pdty(grid,P,R, p_fun, params)>0).*weight ./ sum((profit_qty_x_pdty(grid,P,R, p_fun, params)>0).*weight);
    
    %% Solve for price index
    price_puis = M * mu' * p_fun(grid).^(1-sig);
      
    %% compute the average revenu
    revenu = p_fun(grid).* profit_qty_x_pdty(grid,P,R, p_fun, params);
    rbar = mu' * revenu;
